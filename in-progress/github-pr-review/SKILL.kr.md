...
GitHub pull request를 explicit mode로 review한다. Draft, selected draft finding submit, 또는 user가 명시적으로 opt-in한 YOLO draft-and-submit/approve를 수행한다.
...
YOLO MODE에서 ACTIONABLE FINDING이 0개면 PR을 APPROVE한다.
항상 한국어로 답변.
```
...
- `references/yolo-mode.md` — user inspection 없는 same-run draft and submit/approve workflow.
...
- explicit `yolo` draft-and-submit/approve를 한 번에 수행
...
첫 단어가 정확히 `yolo` 또는 `yolo,`
    └─ YOLO mode: payload approval 없이 draft와 submit/approve를 한 번에 수행
...
YOLO mode는 Draft mode를 실행한 뒤 resulting finding을 submit하거나, actionable finding이 없으면 PR을 approve한다.
...
- same-run draft에서 actionable finding이 없으면 approval 요청 없이 정확히 하나의 `APPROVE` review를 submit한다.

YOLO mode는 같은 YOLO request에서 ID/category include/exclude가 명시되지 않는 한 모든 actionable draft finding을 submit한다. Actionable draft finding이 0개면 comment 제출이나 무동작 대신 PR을 approve한다.
