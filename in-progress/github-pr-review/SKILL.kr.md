...
YOLO MODE에서 ACTIONABLE FINDING이 0개면 APPROVAL이 금지되지 않는 한 PR을 APPROVE하되, self-authored PR은 COMMENT로 fallback한다.
YOLO MODE는 REVIEW EVENT를 판단한다.
YOLO MODE에서 blocking Must Fix가 있으면 REQUEST_CHANGES를 사용하되, self-authored PR은 COMMENT로 fallback한다.
항상 한국어로 답변.
```
...
YOLO mode는 Draft mode를 실행한 뒤 GitHub Review API event를 직접 판단한다. Clean PR은 approve하고, high-confidence blocking Must Fix는 request changes로 제출하며, 그 외에는 comment한다. self-authored PR에서 `APPROVE` 또는 `REQUEST_CHANGES`가 필요하면 review를 `COMMENT`로 제출한다.
...
- same-run draft에서 actionable finding이 없고 validation에서 failing/pending merge-blocking check가 보이지 않으면, 같은 request가 approval을 명시적으로 금지하지 않고 아래 self-authored PR fallback이 적용되지 않는 한 approval 요청 없이 정확히 하나의 `APPROVE` review를 submit한다.
- selected finding 중 high-confidence blocking `Must Fix`가 있으면 아래 self-authored PR fallback이 적용되는 경우를 제외하고 approval 요청 없이 정확히 하나의 `REQUEST_CHANGES` review를 submit한다.
- PR author login이 current viewer login과 같으면 GitHub가 `APPROVE` 및 `REQUEST_CHANGES`를 거절할 수 있다. Event Mode가 self-authored PR에서 둘 중 하나를 선택할 상황이면 `COMMENT`를 사용하고 review content는 계속 제출한다.
