...
이 저장소를 에이전트 런타임 설정 경로(`~/.agents`)에 연결하고 lock-managed 스킬을 설치합니다:

```sh
uv run --script setup_agents.py
```

Unix 계열에서는 실행 권한이 있으면 직접 실행할 수 있습니다. Windows에서는 `uv`로 실행하세요. 심볼릭 링크 권한이 없으면 디렉터리 junction으로 대체합니다.
