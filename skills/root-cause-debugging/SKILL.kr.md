...
description: "버그, 테스트 실패, 빌드/CI 실패, 통합 실패, 플래키, 깨진 UI, 성능저하, 멈춤, 타임아웃, 장애, 이상동작을 고치기 전. Use when debugging a bug, failing test, build/CI failure, integration failure, flaky behavior, broken UI, performance regression, hang, timeout, incident, or unexpected behavior before proposing fixes."
---
