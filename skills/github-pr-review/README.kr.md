...
`github-pr-review`는 GitHub PR review draft, 통제된 review submission, explicit YOLO draft-and-submit/request-changes/approve를 위한 in-progress skill이다.
...
- `references/yolo-mode.md`: classifier가 explicit mode `yolo`를 반환할 때만 enable되는 same-run draft and review-event submit/request-changes/approve workflow.
...
- user inspection gate 없이 explicit `yolo`/`yolo,` draft-and-submit/request-changes/approve를 실행할 때
